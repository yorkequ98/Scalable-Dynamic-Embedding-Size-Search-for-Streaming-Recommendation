...
def min_rating_filter_pandas(
    ...
    """
    split_by_column = _get_column_name(filter_by, 'userID', 'itemID')
...
def split_pandas_data_with_ratios(config, data, ratios, shuffle=False):
...
def filter_k_core(config, data, core_num=5, col_user="userID", col_item="itemID"):
    ...
    print("Final: {} users and {} items".format(num_users, num_items), flush=True)
